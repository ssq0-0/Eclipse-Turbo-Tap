...
async function performAction(
  ...
  const { pause_diapason, pause_time } = config;
  ...
  let nextPauseTime = Date.now() + getRandomNumber(pause_diapason[0], pause_diapason[1]) * 1000;
  ...
  while (Date.now() < startTime + timeWorkMs && retryCount < maxRetries) {
    ...
    if (Date.now() >= nextPauseTime) {
      ...
      nextPauseTime = Date.now() + getRandomNumber(pause_diapason[0], pause_diapason[1]) * 1000;
    }
  }
